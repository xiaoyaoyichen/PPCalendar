...
public class LunarSolarConverter {
    ...
    private final static String[] lFtv = {
            "0101*春节", "0115 元宵节","0505*端午节", "0707 七夕节", "0715 中元节",
    ...
    public static Lunar SolarToLunar(int Year, int Month, int Day) {
        ...
        if(solar.solar24Term != "") {lunar.isLFestival=true;lunar.lunarFestivalName=solar.solar24Term;}
        else {
            findFestival(solar, lunar);
            if (solar.isSFestival) {lunar.isLFestival=true;lunar.lunarFestivalName=solar.solarFestivalName;}
            else if (!lunar.isLFestival) {lunar.isLFestival=false;lunar.lunarFestivalName=getChinaDayString(lunar.lunarDay);}
        }
        return lunar;
    }
